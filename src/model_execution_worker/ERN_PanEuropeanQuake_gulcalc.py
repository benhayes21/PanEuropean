...
def write_loss_streamERN(loss_output_stream, number_of_samples, ouputFile):
    """
    Write loss stream to binary file in format expected by ktools exectuable
    summarycalc.
    ...
    with open(ouputFile, "rb") as itemsOut:
            ...
            while event_id:
                ...
                while True:
                    ...
                    if sidxTemp :
                        ...
                        output_loss.write(struct.pack('i', sidx))
                        output_loss.write(struct.pack('f', loss))
...
def main():
    ...
    #    static_dir
    #)
    # #'-i', '--loss-output-stream'
    # # if args.loss-output-stream is not None:
    # #     do_item_output = True
    # #     if args.loss-output-stream == '-':
    # #         output_item = output_stdout
    # #     else:
    # #         output_item = open(args.loss-output-stream, "wb")
...
def oldgulcalc():
    ...
    parser.add_argument(
        '-a', '--analysis_settings_file', bbrequired=True,
        help='The analysis settings file.',
    )
    parser.add_argument(
    ...
    #rootPath='/home/hiar/Desktop/Examples/ERN_Core/PrA18/'  #root path
    #rutaAME="/usr/local/bin/Catalog V3_7T_LTC_V22.AME" #ame path
    #rutaAME="/usr/local/bin/Case8_LTC_V24.ame" #ame path
    rutaAME="/usr/local/bin/Case9_LTC_V26.ame" #ame path
    coverageTemp= os.path.join(rootPath,'work',os.path.basename(tempfile.NamedTemporaryFile().name))
